...
Future maybeCreateUser(User user) async {
  final userRecord = StoresRecord.collection.doc(user.uid);
  final userExists = await userRecord.get().then((u) => u.exists);
  if (userExists) {
    currentUserDocument = await StoresRecord.getDocumentOnce(userRecord);
    return;
  }

  final userData = createStoresRecordData(
    email: user.email ??
  ...
  currentUserDocument = StoresRecord.getDocumentFromData(userData, userRecord);
}

Future updateUserDocument({String? email}) async {
  await currentUserDocument?.reference
      .update(createStoresRecordData(email: email));
}
